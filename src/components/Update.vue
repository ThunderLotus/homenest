  ...
  <Teleport v-if="data && data.available" to="body">
    <div class="fixed z-20 bottom-[30px] right-[30px] opacity-80 hover:opacity-100 transition-all">
      ...
      <div class="z-1 relative text-end text-fg-dimmed">
        ...
        <a :href="`https://github.com/${githubRepo}/releases/tag/v${data.version}`" class="border-b border-dashed" target="_blank">{{ $t('update.visit') }}</a>
      </div>
    </div>
  </Teleport>
</template>
...
const githubRepo = import.meta.env.HOMENEST_GITHUB_REPO || 'ThunderLotus/homenest'
